...
exports.updateClientRate = async (req, res, next) => {
  ...
  req.flash(
    'success',
    `Successfully updated <strong>${client.clientName}</strong> commingle rate to ${client.clientCommingleRate}`
  );
  res.redirect(`/rateList`);
};
...
exports.updateAgencyRate = async (req, res, next) => {
  ...
  req.flash(
    'success',
    `Successfully updated ${clients.nModified} of ${agency.agencyName} clients' commingle rate to ${
      req.body.rateAmount
    }`
  );
  res.redirect(`/rateList`);
